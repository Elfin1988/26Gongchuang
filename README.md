...
| 引脚 | 当前名称/功能 | 方向 | 当前用途 | 状态 |
| --- | --- | --- | --- | --- |
| `PA0` | `TIM2_CH1` / `MOTOR_PWM` | 输出 | PWM 电机调速输出，蓝线 PWM | 已启用，当前主循环正在使用 |
| `PD3` | `MOTOR_DIR` | 输出 | PWM 电机方向控制，白线正反转 | 已启用，当前主循环正在使用 |
| `PD4` | `MOTOR_FG` / `EXTI4` | 输入 | FG 转速反馈，黄线；电机转一圈输出 6 个方波脉冲 | 已启用，当前中断计数正在使用 |
| `PB14` | `USART1_TX` | 输出 | 串口 1 发送，向上位机发送 `rpm=...\r\n` | 已启用，当前正在使用 |
| `PB15` | `USART1_RX` | 输入 | 串口 1 接收，上位机命令接收预留 | 已配置，当前主循环未启用接收逻辑 |
| `PA4` | `X_STEP` | 输出 | 步进电机 STEP 预留 | 已配置，当前主循环未使用 |
| `PA5` | `X_DIR` | 输出 | 步进电机 DIR 预留 | 已配置，当前主循环未使用 |
